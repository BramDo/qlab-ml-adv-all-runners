#!/usr/bin/env python3
"""Quantum benchmark on the semi-synthetic hard task over real GSE132080 cells."""
...
def render_plot(rows: list[dict[str, object]], *, output_path: str) -> None:
    ...
    ax.set_title("GSE132080 semi-synthetic quantum screen")
    ax.grid(alpha=0.25)
    ax.legend()
    fig.tight_layout()
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run a semi-synthetic GSE132080 quantum benchmark.")
    ...
    parser.add_argument("--encoder", default="direct", choices=["direct", "block", "pca", "spca", "ridge", "lda"])
...
def main() -> None:
    ...
    y_semisynth, semisynth_meta = semisynth.build_residualized_semisynth_labels(
        x_pair,
        guide_y,
    ...
    y_train = y_semisynth[train_idx].astype(np.float64)
    y_test = y_semisynth[test_idx].astype(np.float64)

    ambient_feature_dim = int(comb(int(x_pair.shape[1]), 3))
    ambient_dense_weight_bytes = int(ambient_feature_dim * 8)

    rows: list[dict[str, object]] = []
    json_out = args.json_out or f"qiskit_qos_gse132080_semisynth_quantum_q{args.qubits.replace(',', '_')}.json"
    plot_out = args.plot_out or f"qiskit_qos_gse132080_semisynth_quantum_q{args.qubits.replace(',', '_')}.png"
        ...
        rows.append(
            {
                ...
                "shortcut_hash_seed": (
                    int(args.shortcut_hash_seed) if args.shortcut_hash_seed is not None else int(args.hash_seed)
                ),
                "teacher_dim": int(args.teacher_dim),
                "shortcut_dim_projected_out": int(args.shortcut_dim),
    ...
    payload = {
        ...
        "notes": [
            "Labels are generated on real GSE132080 cells by a hidden third-order teacher whose score is residualized against the natural guide label and a smaller hashed shortcut.",
            "The host expands each cell into hashed third-order interactions and only then feeds a q-dimensional encoded vector into the Qiskit sketch path.",
    ...
    print("GSE132080 semi-synthetic quantum runner")
