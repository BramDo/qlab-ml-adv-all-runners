...
LOGGER = RunLogger("imdb_pca_progress.jsonl")


def get_pca_results_full():
    # 1. Load Data (Full IMDB)
    LOGGER.log("dataset_load_start")
    raw_documents, _ = imdb_utils.load_imdb_data()
    LOGGER.log("dataset_load_done", num_documents=int(len(raw_documents)))

    # 2. Compute "Ground Truth" (Full Dimension)
    LOGGER.log("reference_vectorize_start", min_df=1)
    ...
    LOGGER.log(
        "reference_vectorize_done",
        num_samples=int(X_full.shape[0]),
        feature_dim=int(X_full.shape[1]),
        nnz=int(X_full.getnnz()),
    )

    LOGGER.log("reference_svds_start", k=1)
    ...
    LOGGER.log("reference_svds_done", k=1)
    ...
    for mdf in tqdm(min_dfs, desc="min_df Sweep"):
        LOGGER.log("min_df_start", min_df=int(mdf))
        ...
        LOGGER.log(
            ...
            nnz=int(space_sparse),
            row_sparsity=int(row_sparsity),
            col_sparsity=int(col_sparsity),
            space_streaming=float(space_stream),
            space_sparse=float(space_sparse),
            space_quantum=float(space_quantum),
        )

        # --- SVD Calculation (Sparse) ---
        # Compute Top Singular Vector in Truncated Space
        LOGGER.log("min_df_svds_start", min_df=int(mdf), k=1)
        _, _, vt_trunc = svds(X_trunc.asfptype(), k=1)
        v_trunc = vt_trunc[0]  # type: ignore # Shape (D,)
        LOGGER.log("min_df_svds_done", min_df=int(mdf), k=1)
        ...
        LOGGER.log(
            "min_df_done",
            min_df=int(mdf),
            variance_recovery=float(recovery),
        )
...
def run_analysis(load_file=None):
    ...
    if load_file is not None:
        LOGGER.log("analysis_start", mode="load_file", path=str(load_file))
        ...
        LOGGER.log("analysis_start", mode="fresh")
        ...
        LOGGER.log(
            "json_saved",
            path="imdb_size_vs_variance.json",
            num_points=int(len(results["min_dfs"])),
        )
    ...
    LOGGER.log("pdf_saved", path="imdb_size_vs_variance.pdf")
