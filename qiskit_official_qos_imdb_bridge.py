#!/usr/bin/env python3
"""Small real-data bridge from official IMDb to the Qiskit QOS kernel port."""
...
def compute_imdb_space_metrics(x) -> dict[str, float | int]:
    shape = x.get_shape()
...
def load_curve_point(official_json: Path, min_df: int, metric_key: str) -> dict[str, float] | None:
    ...
    return {
        ...
        metric_key: float(point["streaming"][metric_key]),
    }


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Bridge official IMDb data into the small Qiskit QOS kernel port"
    )
    ...
    parser.add_argument(
        ...
        type=Path,
        default=OFFICIAL_REAL_DATASETS / "imdb_size_vs_accuracy.json",
    )
    parser.add_argument(
        "--official-variance-json",
        type=Path,
        default=OFFICIAL_REAL_DATASETS / "imdb_size_vs_variance.json",
    )
...
def main() -> None:
    ...
    payload = {
        "dataset": "IMDb Full binary sentiment",
        "label_names": label_names,
        "min_df": args.min_df,
        "full_shape": [int(len(raw_documents)), int(x_filtered.shape[1])],
        "class_balance": {
            str(label_names[0]): int(np.sum(np.asarray(y) == 0)),
            str(label_names[1]): int(np.sum(np.asarray(y) == 1)),
        },
        "paper_space_metrics_pair_specific": compute_imdb_space_metrics(x_filtered),
        "official_curve_reference_at_min_df_accuracy": load_curve_point(
            args.official_accuracy_json, args.min_df, "accuracy_mean"
        ),
        "official_curve_reference_at_min_df_variance": load_curve_point(
            args.official_variance_json, args.min_df, "variance_recovery"
        ),
